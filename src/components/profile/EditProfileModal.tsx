...
import type { UserProfile, EditProfileForm, SocialLink } from '../../types/profile';
...
function TagInput({ tags, onAdd, onRemove, placeholder }: {
  ...
  return (
    <div className="min-h-[44px] border border-[#e4e5e8] bg-white rounded-full px-3 flex flex-wrap items-center gap-1.5 py-1.5">
      {tags.map((t) => (
        <span key={t} className="flex items-center gap-1 bg-[#ffeacc] text-[#ff9400] text-xs rounded-full pl-2.5 pr-1.5 py-1">
          {t}
...
const inputCls = 'w-full h-[44px] border border-[#e4e5e8] rounded-full px-4 text-sm text-[#18191c] placeholder-[#9199a3] focus:outline-none focus:border-[#ff9400] focus:ring-1 focus:ring-[#ff9400] transition-colors bg-white';
const labelCls = 'text-[#374151] text-sm font-medium mb-1.5 block';

export default function EditProfileModal({ profile, saving, onSave, onClose }: Props) {
  ...
  const [activeSection, setActiveSection] = useState<'basic' | 'skills' | 'contact'>('basic');
  ...
  function addSocialLink() {
    set('socialLinks', [...form.socialLinks, { platform: 'LinkedIn', url: '' }]);
  }
  ...
  function updateSocialLink(idx: number, field: keyof SocialLink, value: string) {
    set('socialLinks', form.socialLinks.map((l, i) => i === idx ? { ...l, [field]: value } : l));
  }
    ...
    >
      <div className="bg-white rounded-[20px] w-full max-w-[600px] max-h-[90vh] flex flex-col shadow-[0px_20px_60px_rgba(0,0,0,0.2)]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#f2f2f3] shrink-0">
          ...
          <button
            onClick={onClose}
            className="text-[#9ca3af] hover:text-[#374151] transition-colors rounded-full p-1 focus:outline-none focus:ring-2 focus:ring-[#ff9400]"
            aria-label="Close"
          >
          ...
          {activeSection === 'basic' && (
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                <input type="text" placeholder="e.g. UI/UX Designer" value={form.title}
                  onChange={(e) => set('title', e.target.value)} className={inputCls} />
              </div>
              <div>
                <label className={labelCls}>Company / Organisation</label>
                <input type="text" placeholder="e.g. Accenture" value={form.company}
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full border border-[#e4e5e8] rounded-[14px] px-4 py-3 text-sm text-[#18191c] placeholder-[#9199a3] focus:outline-none focus:border-[#ff9400] focus:ring-1 focus:ring-[#ff9400] transition-colors resize-none"
                />
              </div>
            </div>
          )}
          ...
          {activeSection === 'skills' && (
            <div className="flex flex-col gap-5">
              <div>
                ...
                <p className="text-[#9199a3] text-xs mb-2">Type and press comma or Enter to add</p>
                <TagInput
                  tags={form.skills}
                  onAdd={(t) => set('skills', [...form.skills, t])}
                  onRemove={(t) => set('skills', form.skills.filter((s) => s !== t))}
                  placeholder="Add a skill..."
                />
              </div>
              <div>
                <label className={labelCls}>Reach Out to Me For</label>
                <p className="text-[#9199a3] text-xs mb-2">Topics or areas people can reach out to you for</p>
                <TagInput
                  tags={form.reachFor}
                  onAdd={(t) => set('reachFor', [...form.reachFor, t])}
                  onRemove={(t) => set('reachFor', form.reachFor.filter((s) => s !== t))}
          ...
          {activeSection === 'contact' && (
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                <input type="text" placeholder="e.g. English, Hindi" value={form.languages}
                  onChange={(e) => set('languages', e.target.value)} className={inputCls} />
              </div>
              <div>
                  ...
                  {form.socialLinks.map((link, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      <select
                        ...
                        className="h-[44px] border border-[#e4e5e8] rounded-full px-3 text-sm text-[#18191c] bg-white focus:outline-none focus:border-[#ff9400] w-[150px] shrink-0"
                      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-[#f2f2f3] shrink-0">
          ...
          <button
            ...
            disabled={saving}
            className="h-[42px] px-8 bg-[#ff9400] text-white text-sm font-semibold rounded-full hover:bg-[#e68500] transition-colors shadow-[0px_4px_12px_rgba(255,148,0,0.3)] disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2"
          >
            {saving && (
