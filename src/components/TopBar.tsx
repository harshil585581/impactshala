...
function NotifAvatar({ notif }: { notif: AppNotification }) {
...
export default function TopBar({ onMenuToggle }: TopBarProps) {
  ...
  const { notifications, unreadCount, dismiss, dismissAll, markRead } = useNotifications();
            ...
            <div className="absolute right-0 top-[calc(100%+12px)] w-[340px] sm:w-[380px] bg-white rounded-2xl shadow-[0px_8px_32px_rgba(0,0,0,0.14)] border border-[#f2f2f3] z-50 overflow-hidden
              ...
              <div className="flex items-center justify-between px-4 pt-4 pb-2">
                ...
                {notifications.length > 0 && (
                  <button
                    onClick={() => { dismissAll(); }}
                    className="text-[#9199a3] hover:text-[#ff9400] text-xs font-semibold transition-colors"
                  >
                    Clear all
                  </button>
                )}
              ...
              <div className="max-h-[360px] overflow-y-auto divide-y divide-[#f2f2f3]">
                {notifications.length === 0 ? (
                  ...
                  notifications.map((n) => (
                    <div
                      key={n.id}
                      className="flex items-start gap-3 px-4 py-3 hover:bg-[#fafafa] group transition-colors cursor-pointer"
                      onClick={() => markRead(n.id)}
                    >
                      <NotifAvatar notif={n} />
                      <div className="flex-1 min-w-0">
                        <p className="text-[#18191c] text-xs leading-snug">{n.message}</p>
