      ...
      <div className="bg-white rounded-[20px] w-full max-w-md shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-8 pt-7 pb-4">
          ...
          <button onClick={onClose} className="text-[#18191c] hover:opacity-70 transition-opacity">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M18 6L6 18M6 6l12 12" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" /></svg>
          </button>
        </div>
        ...
        <div className="px-8 py-5 flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            ...
            <input className={inp} placeholder="Ex: Social Services" value={name} onChange={e => setName(e.target.value)} />
          </div>
          <div className="flex flex-col gap-2">
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function PublicWelfareServicesOrgProfilePage() {
  ...
  const { profile, loading, toasts, removeToast, addToast, saveProfile, reload } = useProfile(userId);
  ...
  const welfareTypes = profile?.applicableIndustries ?? [];

  // Show-more logic for welfare types row
  const welfareMeasureRef = useRef<HTMLDivElement>(null);
  const [welfareLimit, setWelfareLimit] = useState(welfareTypes.length || 20);

  useEffect(() => {
    if (!welfareMeasureRef.current) return;
    const observer = new ResizeObserver(() => {
      const container = welfareMeasureRef.current;
      if (!container) return;
      ...
      setWelfareLimit(wrapIndex !== -1 ? Math.max(1, wrapIndex - 1) : welfareTypes.length);
    });
    observer.observe(welfareMeasureRef.current);
    return () => observer.disconnect();
  }, [welfareTypes.join(',')]);

  const sectorLabel = profile?.sector === 'government' ? 'Government Sector'
    : profile?.sector === 'private' ? 'Private Sector'
      : profile?.sector === 'public' ? 'Public Sector'
        : profile?.sector ?? '';
  const subtitleParts = [sectorLabel, 'Public Welfare Services'].filter(Boolean);
        ...
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 py-6">
          {loading ? (
            ...
            <div className="bg-white rounded-[14px] border border-[#f2f2f3] shadow-[0px_2px_8px_rgba(0,0,0,0.06)]">
              ...
              {/* Cover */}
              <div className="relative w-full h-[170px] sm:h-[230px] bg-gradient-to-r from-[#3a2a10] via-[#7a5a20] to-[#3a2a10] rounded-t-[14px] overflow-hidden">
                {profile?.coverUrl && (
              ...
              <div className="px-6 sm:px-16 py-6 flex flex-col">
                ...
                {welfareTypes.length > 0 && (
                  <>
                    <div>
                      <p className="text-[#18191c] text-base font-bold mb-3">Welfare Services</p>
                      {showAllWelfare ? (
                        <>
                          <div className="flex flex-wrap gap-2">
                            {welfareTypes.map((tag) => <OutlinedTag key={tag} label={tag} />)}
                          </div>
                          <button onClick={() => setShowAllWelfare(false)} className="mt-3 text-[#ff9400] text-sm font-medium hover:underline">
                            Show less
                          </button>
                        </>
                      ) : (
                        <div className="relative">
                          <div ref={welfareMeasureRef} className="absolute top-0 left-0 right-0 opacity-0 pointer-events-none flex flex-wrap gap-2 max-h-[46px] overflow-hidden">
                            {welfareTypes.map((tag) => <OutlinedTag key={tag} label={tag} />)}
                          </div>
                          <div className="flex items-center gap-2 flex-wrap">
                            {welfareTypes.slice(0, welfareLimit).map((tag) => (
                              <OutlinedTag key={tag} label={tag} />
                            ))}
                            {welfareLimit < welfareTypes.length && (
                              <button
                                onClick={() => setShowAllWelfare(true)}
