...
const HOSPITAL_FACILITY_TYPES = [
  'General Hospitals',
  'Maternity Homes',
  'Wellness & Naturopathy Centers',
  'Multi-Specialty Hospitals',
  'Child & Women Wellness Centers',
  'Blood Banks',
  ...
  'Vaccination/Immunization Centers',
  'Polyclinics / Specialty Clinics',
  'Rehabilitation Centers',
  'Nursing Homes',
  'Diagnostic Centers',
  'Palliative Care Centers',
  'AYUSH Clinics (Ayurveda, Yoga, Unani, Siddha, Homeopathy)',
  'Pathology Labs',
        ...
        <div className="absolute z-50 top-[calc(100%+10px)] left-0 bg-white rounded-2xl shadow-[0px_8px_32px_rgba(0,0,0,0.12)] border border-[#f2f2f3] overflow-hidden min-w-[190px]">
          {SOCIAL_PLATFORMS.map((p) => (
            <button
              ...
              className={`w-full text-left px-5 py-3 text-sm flex items-center gap-2.5 transition-colors ${
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function HealthServicesOrgAccountUpdatePage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/profile`, {
      ...
      .then((d) => {
        ...
        if (d.applicable_industries?.length) setFacilityTypes(d.applicable_industries);
  ...
  function toggleFacility(type: string) {
  ...
  async function handleSave() {
    ...
    const payloadByTab: Record<string, object> = {
      org:     { sector, applicable_industries: facilityTypes, bio },
          ...
          <div className="max-w-[960px] mx-auto px-4 sm:px-6 py-6 sm:py-8">
            ...
            {tab === 'org' && (
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-3">
                  <p className="text-[#18191c] text-sm font-semibold">Select Your Applicable Hospital Facility Types</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-3">
                    {HOSPITAL_FACILITY_TYPES.map((type) => (
                      <label key={type} className="flex items-start gap-2.5 cursor-pointer group">
                        <div
                          onClick={() => toggleFacility(type)}
                          className={`w-4 h-4 mt-0.5 shrink-0 border rounded flex items-center justify-center transition-colors cursor-pointer ${
