...
export default function EntrepreneurAccountUpdatePage() {
  ...
  useEffect(() => {
    ...
    if (profile.entrepreneur_type) setEntrepreneurType(profile.entrepreneur_type);
  ...
  async function handleSave() {
    if (tab === 'profile') {
      const ok = await save({ entrepreneur_type: entrepreneurType, describe_as: describeAs, bio });
      if (ok) setTab('more');
    } else if (tab === 'more') {
