...
const calendarIcon = "https://www.figma.com/api/mcp/asset/cf259d59-49ad-45ed-850d-b566af53d11e";
          ...
          <div className="w-full max-w-[576px]">
            ...
            <div className="bg-white sm:rounded-[23px] sm:shadow-[0px_4px_3px_rgba(0,0,0,0.07),0px_10px_20px_rgba(0,0,0,0.08)] px-0 sm:px-6 md:px-8 py-0 sm:py-8">
              ...
              <div className="text-center mb-5 sm:mb-6">
                ...
                <p className="text-[#4b5563] text-sm sm:text-base mt-1">Join our community today</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                {/* Date of birth */}
                <div>
                  <label className="block text-[#374151] text-sm font-medium mb-1.5">Date of birth</label>
                ...
                {/* Interest Areas */}
                <div className="relative">
                  <label className="block text-[#374151] text-sm font-medium mb-1.5">Select Your Interest Areas</label>
                  <button
                    type="button"
                    onClick={() => setShowInterests((v) => !v)}
                    className="w-full h-[48px] sm:h-[50px] border border-[#d6d6d6] rounded-full px-4 text-left flex items-center justify-between hover:border-[#ff9400] transition-colors focus:outline-none focus:border-[#ff9400] bg-white"
                  >
                    <span className={`text-sm truncate pr-2 ${form.interests ? 'text-[#1e1e1e]' : 'text-[#adaebc]'}`}>
                    ...
                    >
                      <path d="M6 9l6 6 6-6" stroke="#adaebc" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </button>
                  {showInterests && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-[#e5e7eb] rounded-2xl shadow-lg z-10 overflow-hidden">
                      {interestOptions.map((opt) => (
                        <button
                          key={opt}
                          type="button"
                          onClick={() => { update('interests', opt); setShowInterests(false); }}
                          className="w-full text-left px-4 py-3 text-sm text-[#1e1e1e] hover:bg-[#fff6ed] hover:text-[#ff9400] transition-colors"
                        >
                          {opt}
                        </button>
                      ))}
