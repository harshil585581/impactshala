  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      <div className="flex justify-center px-5 pt-8 pb-4">
        <img src={primaryLogo} alt="Impactshaala" className="h-9 w-auto object-contain" />
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-5 py-12">
        <div className="flex flex-col items-center gap-6 text-center max-w-[380px]">
          {status === 'verifying' && (
            <>
              <div className="w-14 h-14 border-4 border-[#ff9400] border-t-transparent rounded-full animate-spin" />
            ...
            <>
              <div className="bg-[#fff8f0] rounded-full p-6">
                <svg width="56" height="56" viewBox="0 0 24 24" fill="none">
                  <circle cx="12" cy="12" r="12" fill="#ff9400" />
                  <path d="M7 12l4 4 6-7" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
              <h2 className="text-[#0f172a] text-xl font-semibold">Email Verified!</h2>
              <p className="text-[#64748b] text-sm">Redirecting you to the app…</p>
            </>
          )}
          ...
          {status === 'error' && (
            <>
              ...
              <h2 className="text-[#0f172a] text-xl font-semibold">Verification Failed</h2>
              <p className="text-[#64748b] text-sm">{errorMsg}</p>
              <button
                onClick={() => navigate('/signup/verify')}
                className="w-full h-[50px] bg-[#ff9400] text-white text-sm font-bold rounded-full hover:bg-[#e68500] transition-colors"
              >
