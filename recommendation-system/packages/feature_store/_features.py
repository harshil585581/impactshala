...
# Event type weights for user embedding computation
...
class FeatureService:
    ...
    async def get_user_embedding(self, user_id: str) -> np.ndarray:
        """Return 384-dim user embedding from cache or compute on-the-fly."""
        ...
        await self.redis.setex(
            cache_key,
            self.user_embedding_ttl,
            json.dumps(embedding.tolist()),
        )
        return embedding

    async def _compute_user_embedding(self, user_id: str) -> np.ndarray:
        """Weighted sum of content embeddings from user interactions."""
        from packages.models.database import Event, Content

        # Fetch last 90 days of events with content embeddings
        ...
        if not rows:
            # Return random unit vector for cold-start users
    ...
    async def update_user_embedding_online(
        ...
        """Online learning step: nudge user embedding toward/away from content."""
    ...
    async def get_content_embedding(self, content_id: str) -> np.ndarray:
        """Return 384-dim sentence embedding for the content item."""
    ...
    async def get_social_graph_signals(self, user_id: str) -> Dict[str, Any]:
        """Return content engaged by followed users in the last 48 hours."""
        from packages.models.database import Follow, Event

        # 1. Get all followees
        followee_result = await self.db.execute(
            select(Follow.followee_id, Follow.entity_type)
        ...
        if not followed_user_ids:
            return {"followed_content_ids": [], "trending_in_network": [], "second_degree_ids": []}

        since = datetime.now(timezone.utc) - timedelta(hours=48)

        # 2. Content engaged by direct follows
        ...
        # 3. Trending in network (most engaged)
        ...
        # 4. Second-degree: users followed by our followees
    ...
    async def get_interest_graph(self, user_id: str) -> Dict[str, float]:
        """Return normalized topic affinity scores for the user."""
    ...
    async def _compute_interest_graph(self, user_id: str) -> Dict[str, float]:
        ...
        # Events with dwell/type signals
        ...
        # Explicit topic follows ×2.0
        follow_result = await self.db.execute(
            select(Follow.followee_id)
            .where(
                and_(
                    Follow.follower_id == user_id,
                    Follow.entity_type == "topic",
                )
            )
        )
    ...
    async def get_real_time_session_signals(self, session_id: str) -> Dict[str, Any]:
        """Extract intent and signals from the active session's event window."""
        ...
        momentum = (positive_count - negative_count) / max(session_len, 1)
    ...
    async def push_session_event(
        ...
        """Maintain a sliding window of events for the session in Redis."""
        session_key = f"session:{session_id}"
